...
export default async function CalculatorsPage() {
  ...
  const calcs = [
    ...
    { href: "/calculators/affordability", title: t("affordability.title"), body: t("affordability.description") },
  ];
        ...
        {calcs.map((c) => (
          <Link key={c.href} href={c.href} className="group">
            <Card className="h-full transition-shadow group-hover:shadow-md">
              ...
              <p className="mt-2 text-sm text-stone-600 dark:text-stone-300">{c.body}</p>
            </Card>
          </Link>
        ))}
      </div>
